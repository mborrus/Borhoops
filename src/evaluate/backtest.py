...
def load_tourney_results(data_dir, gender, year):
    """Load tournament results for one gender/year."""
...
def _run_elo_collecting_preds(results, conferences, hfa_dict, location_dict,
    ...
    Same logic as run_elo(), but also records the model's prediction for each game
    in `predict_seasons` *before* the Elo update — giving us honest out-of-sample preds.

    Returns:
        (elo_dict, preds_list, outcomes_list)
    """
    ...
    for season in seasons:
        ...
        for _, row in season_games.iterrows():
            ...
            # Record pre-game prediction (lower-TeamID perspective)
            if collecting:
...
def backtest_tourney(data, gender, year):
    """Run Elo through season `year`, predict that year's tournament, return scored results."""
...
def _load_models(model_dir, gender):
    """Load all available models for a gender. Returns {name: model}."""
...
def backtest_tourney_ml(data, gender, year, model_dir):
    ...
    # Run Elo through end of season to get ratings + game counts
    ...
    # Build feature matrix for tournament games
...
def _print_results(label, result):
    """Print Brier score and calibration for one backtest result."""
    print(f"\n=== {label} ===")
...
def _print_ml_results(label, elo_result, ml_results):
    """Print Elo + ML model Brier scores side by side."""
    print(f"\n=== {label} ===")
    if elo_result:
